/*
 * lkf-map-maker-opt.hpp
 *
 *  Created on: 2013/09/27
 *      Author: tyamada
 */

#ifndef LKF_MAP_MAKER_OPT_HPP_
#define LKF_MAP_MAKER_OPT_HPP_
...
#include "lkf-map-maker-conf.hpp"

namespace lkf {
	namespace map_maker {

		const char conf_fname[] = "lkf-map-maker.conf";
		...
		inline bool options::get_option(int aArgc, char **aArgv)
		{
			...
			while(1){
				...
				// write configure
				case 'G': {
					proc_conf_write( optarg ? optarg : conf_fname, pconf);
					::fprintf(stderr, " ... output configuration file \"\x1b[4m%s\x1b[0m\"\n", optarg ? optarg : conf_fname);
				} return RWriteConf;
